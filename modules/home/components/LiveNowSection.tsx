      ...
      <div className="flex gap-3 overflow-x-auto no-scrollbar px-5 snap-x snap-mandatory">
        {eventos.map(e => (
          <div
            ...
            className="shrink-0 w-[75vw] max-w-[20rem] snap-start bg-[#161616] border border-red-500/20 rounded-2xl overflow-hidden flex gap-3 p-3 cursor-pointer active:scale-[0.98] transition-transform shadow-[0_0_20px_rgba(239,68,68,0.1)]"
          >
            <div className="w-16 h-16 rounded-xl overflow-hidden shrink-0 relative">
              ...
              <div className="absolute top-1 left-1 flex items-center gap-1 bg-red-500/90 backdrop-blur px-1.5 py-0.5 rounded-full">
                ...
                <span className="text-[0.4375rem] font-black text-white uppercase tracking-wider">Live</span>
              </div>
            </div>
            <div className="flex-1 min-w-0 flex flex-col justify-center">
              <span className="text-[0.5625rem] font-bold text-red-400 uppercase tracking-wider mb-0.5 truncate">
                {e.formato || e.categoria}
              </span>
              <h4 className="font-serif text-sm text-white leading-tight truncate">{e.titulo}</h4>
              <p className="text-zinc-400 text-[0.625rem] mt-0.5 truncate">{e.local}</p>
            </div>
            <div className="flex items-center shrink-0">
              <ArrowRight size="0.875rem" className="text-red-400" />
            </div>
          </div>
        ))}
      </div>
