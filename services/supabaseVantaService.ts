...
export class SupabaseVantaService implements IVantaService {
  ...
  // ── checkinLista ────────────────────────────────────────────────────────────
  async checkinLista(convidadoId: string, porteiroNome?: string): Promise<{ ok: boolean; checkedInEm?: string }> {
    const svc = await getListasService();
...
// ── uploadBiometria ────────────────────────────────────────────────────────────
/**
 * Faz upload da selfie biométrica para o Supabase Storage.
 *
 * Caminho: selfies/{userId}/biometria.jpg
 * Bucket : selfies (público para leitura; restrito para escrita por RLS)
 *
 * Retorna:
 *   { path, publicUrl } em caso de sucesso
 *   { path: null, error } em caso de falha — UI usa fallback localStorage (base64)
 *
 * @param base64DataUrl  data URL da selfie (ex: "data:image/jpeg;base64,...")
 * @param userId         ID do membro recém-cadastrado
 */
export interface UploadBiometriaResult {
  path: string | null;
  publicUrl: string | null;
  error?: string;
}

export const uploadBiometria = async (base64DataUrl: string, userId: string): Promise<UploadBiometriaResult> => {
  try {
    // Converte data URL → Blob
    const res = await fetch(base64DataUrl);
    const blob = await res.blob();
    const file = new File([blob], 'biometria.jpg', { type: 'image/jpeg' });

    const path = `${userId}/biometria.jpg`;
    ...
    if (uploadError) {
      console.warn('[SUPABASE] uploadBiometria falhou — usando fallback localStorage:', uploadError.message); // audit-ok: error handling
    ...
    console.warn('[SUPABASE] uploadBiometria exception — fallback localStorage:', msg);
    return { path: null, publicUrl: null, error: msg };
  }
};
