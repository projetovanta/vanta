...
export const SubTabEventos: React.FC<Props> = ({
  ...
  const [reservasEvento, setReservasEvento] = useState<ResgateMV[]>([]);
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    getResgatesEvento(eventoSelecionado).then(data => {
      if (!cancelled) setReservasEvento(data);
    });
    return () => {
      cancelled = true;
    };
  ...
  return (
    <div className="space-y-3">
      ...
      {eventoSelecionado &&
        (() => {
          // reservasEvento já carregado via useEffect
