    ...
    if (tipo === 'INGRESSOS') {
      return (
        <div className="space-y-3">
          <p className="text-[0.5rem] text-zinc-400 font-black uppercase tracking-widest">Por evento</p>
          {eventos.map(e => {
              ...
              <div key={e.id} className="bg-zinc-900/40 border border-white/5 rounded-2xl p-4">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-2.5">
                  <div className="text-center">
                    ...
                    <p className="text-zinc-400 text-[0.5625rem] font-bold uppercase tracking-wider">Total</p>
                  </div>
                  <div className="text-center">
                    <p className="text-blue-400 font-black text-xl">{mascVar}</p>
                    <p className="text-zinc-400 text-[0.5625rem] font-bold uppercase tracking-wider">Masc.</p>
                  </div>
                  <div className="text-center">
                    <p className="text-pink-400 font-black text-xl">{femVar}</p>
                    <p className="text-zinc-400 text-[0.5625rem] font-bold uppercase tracking-wider">Fem.</p>
                  </div>
                </div>
              </div>
            );
      ...
      return (
        <div className="space-y-4">
          {topPromoters.length > 0 && (
            <>
              <p className="text-[0.5rem] text-zinc-400 font-black uppercase tracking-widest">
                Quem mais adicionou nomes
              </p>
              <div className="space-y-2">
                {topPromoters.map((p, i) => (
                  <div key={i} className="flex items-center gap-3 p-3 bg-zinc-900/40 border border-white/5 rounded-xl">
                    <span
                      className={`w-6 h-6 rounded-full flex items-center justify-center text-[0.625rem] font-black shrink-0 ${i === 0 ? 'bg-[#FFD300] text-black' : 'bg-zinc-800 text-zinc-400'}`}
                    >
                      {i + 1}
                    </span>
                    <p className="text-white text-sm font-bold flex-1 min-w-0 truncate">{p.nome}</p>
                    <p className="text-[#FFD300] font-black text-sm">
                      {p.total} <span className="text-zinc-400 text-[0.5625rem] font-normal">nomes</span>
                    </p>
                  </div>
                ))}
              </div>
          ...
          {listaMatches.length === 0 && (
            <p className="text-zinc-700 text-[0.625rem] italic text-center py-4">Nenhuma lista vinculada.</p>
          )}
            ...
            <div key={evento.id} className="bg-zinc-900/40 border border-white/5 rounded-2xl p-4">
              ...
              <div className="grid grid-cols-2 gap-2">
                <div className="p-2.5 bg-zinc-900 rounded-xl text-center">
                  ...
                  <p className="text-zinc-400 text-[0.5rem] font-black uppercase tracking-widest">Na lista</p>
                </div>
                <div className="p-2.5 bg-emerald-500/5 border border-emerald-500/10 rounded-xl text-center">
                  ...
                  <p className="text-zinc-400 text-[0.5rem] font-black uppercase tracking-widest">Entraram</p>
                </div>
              </div>
            </div>
          ))}
    ...
    if (tipo === 'LOTES') {
      ...
      return (
        <div className="space-y-2">
          {sorted.map((lote, i) => (
            <div key={lote.id} className="flex items-center gap-3 p-3 bg-zinc-900/40 border border-white/5 rounded-xl">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-zinc-400 text-[0.5625rem] truncate">{lote.eventoNome}</p>
              </div>
          ...
          {sorted.length === 0 && (
            <p className="text-zinc-700 text-[0.625rem] italic text-center py-8">Nenhum dado de vendas.</p>
          )}
        </div>
      );
    }
