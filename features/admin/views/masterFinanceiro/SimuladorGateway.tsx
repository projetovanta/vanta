  ...
  return (
    <div className="bg-zinc-900/40 border border-white/5 rounded-2xl p-5 space-y-4">
      <div className="flex items-center gap-2">
        <Calculator size="0.8125rem" className="text-[#FFD300] shrink-0" />
        <p className="text-[0.5rem] text-zinc-400 font-black uppercase tracking-widest">Simulador de Lucro Real</p>
      </div>

      <div className="grid grid-cols-2 gap-3">
        <div className="space-y-1.5">
          <p className="text-[0.5rem] text-zinc-700 font-black uppercase tracking-widest">Valor Ingresso (R$)</p>
          <input
            value={simValor}
        ...
        <div className="space-y-1.5">
          <p className="text-[0.5rem] text-zinc-700 font-black uppercase tracking-widest">Método</p>
          <div className="flex gap-1.5">
            {(['CREDITO', 'PIX'] as const).map(m => (
              <button
                key={m}
                onClick={() => setSimMetodo(m)}
                className={`flex-1 py-2.5 rounded-xl text-[0.5625rem] font-black uppercase tracking-wider transition-all flex items-center justify-center gap-1 ${
                  simMetodo === m
                    ? 'bg-[#FFD300]/10 border border-[#FFD300]/30 text-[#FFD300]'
                    : 'bg-zinc-900/60 border border-white/5 text-zinc-400'
                }`}
              >
                {m === 'PIX' ? <Zap size="0.625rem" /> : <CreditCard size="0.625rem" />}
                {m === 'PIX' ? 'Pix' : 'Cartão'}
              </button>
            ))}
          </div>
      ...
      {simValorNum > 0 && (
        <div className="space-y-2 pt-3 border-t border-white/5">
          <div className="space-y-1">
            <div className="flex justify-between text-[0.5625rem] pt-1 border-t border-white/5">
          ...
          <div className="grid grid-cols-2 gap-2 pt-2">
            <div className="bg-zinc-900/60 border border-white/5 rounded-xl p-3 text-center">
              <p className="text-zinc-400 text-[0.5rem] font-black uppercase tracking-widest mb-1">100 ingressos</p>
              <p className="text-zinc-300 font-black text-sm leading-none">{fmtBRL(simLucro100)}</p>
            </div>
            <div className="bg-zinc-900/60 border border-white/5 rounded-xl p-3 text-center">
              <p className="text-zinc-400 text-[0.5rem] font-black uppercase tracking-widest mb-1">500 ingressos</p>
