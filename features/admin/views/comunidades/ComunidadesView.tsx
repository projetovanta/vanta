      ...
      <div className="shrink-0 bg-[#0A0A0A] border-b border-white/5 px-6 pt-6 pb-4 flex justify-between items-start">
        ...
        <div className="flex items-center gap-2 mt-1">
          {adminRole === 'vanta_masteradm' && (
            ...
            >
              <Plus size="0.875rem" className="text-black" />
              <span className="text-black font-black text-[0.625rem] uppercase tracking-wider">Nova</span>
            </button>
          )}
          <button
            aria-label="Voltar"
          ...
          >
            ...
            {!c.ativa && (
              <div className="absolute top-4 right-4 bg-black/60 backdrop-blur-md border border-white/10 rounded-full px-3 py-1">
                <p className="text-zinc-400 text-[0.5rem] font-black uppercase tracking-widest">Inativa</p>
              </div>
            )}
            <div className="absolute bottom-0 left-0 right-0 p-5">
              <div className="flex items-center gap-1 mb-1">
                <MapPin size="0.5rem" className="text-[#FFD300]/60" />
                <p className="text-[#FFD300]/60 text-[0.5625rem] font-black uppercase tracking-[0.2em]">{c.cidade}</p>
              </div>
              ...
              <div className="flex gap-2">
                <div className="flex items-center gap-1.5 bg-white/10 backdrop-blur-sm rounded-full px-3 py-1">
                  <Calendar size="0.5625rem" className="text-white/60" />
                  <p className="text-white text-[0.625rem] font-bold">
                    {c.eventoIds.length} evento{c.eventoIds.length !== 1 ? 's' : ''}
                  </p>
                </div>
                <div className="flex items-center gap-1.5 bg-white/10 backdrop-blur-sm rounded-full px-3 py-1">
                  <Users size="0.5625rem" className="text-white/60" />
                  {(() => {
