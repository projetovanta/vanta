...
const WaitlistSection: React.FC<{ listaId: string }> = ({ listaId }) => {
  ...
  return (
    <>
      <p className="text-zinc-700 text-[0.5625rem] font-black uppercase tracking-widest px-1 flex items-center gap-1.5">
        <Bell size="0.625rem" className="text-amber-400" /> Lista de Espera
      </p>
      {entries.map(([varId, count]) => (
        ...
        >
          ...
          <span className="px-2.5 py-1 bg-amber-500/10 border border-amber-500/20 rounded-full text-amber-400 text-[0.5625rem] font-black shrink-0">
            {count} na fila
          </span>
        </div>
      ))}
