/**
 * clubeLotesService — CRUD para mais_vanta_lotes_evento.
 * Liga tier MAIS VANTA → lote real ou lista real de um evento.
 */
import { supabase } from '../../../../services/supabaseClient';
import type { Database } from '../../../../types/supabase';

type Row = Database['public']['Tables']['mais_vanta_lotes_evento']['Row'];
type Insert = Database['public']['Tables']['mais_vanta_lotes_evento']['Insert'];
...
export async function salvarBeneficiosEvento(
  ...
  // Remover existentes
  const { error: delErr } = await supabase.from('mais_vanta_lotes_evento').delete().eq('evento_id', eventoId);
  if (delErr) {
  ...
  const { data, error } = await supabase.from('mais_vanta_lotes_evento').insert(rows).select('*');
  if (error) {
...
export async function removerBeneficiosEvento(eventoId: string): Promise<void> {
  const { error } = await supabase.from('mais_vanta_lotes_evento').delete().eq('evento_id', eventoId);
