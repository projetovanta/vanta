  ...
  <div className="absolute inset-0 z-50 flex items-end justify-center bg-black/85">
    ...
    >
      ...
      <p className="text-zinc-400 text-sm text-center leading-relaxed mb-4">
        ...
        <span
          className={`text-[10px] font-black uppercase tracking-wider px-2 py-0.5 rounded ${selectedDestino.tipo === 'COMUNIDADE' ? 'bg-blue-500/20 text-blue-400' : 'bg-purple-500/20 text-purple-400'}`}
        >
      ...
      <div className="space-y-2">
        <button
          onClick={onConfirmar}
          className="w-full py-4 bg-[#FFD300] text-black font-bold text-[10px] uppercase tracking-[0.3em] rounded-xl active:scale-[0.98] transition-all"
        >
          Confirmar
        </button>
        <button
          onClick={onCancelar}
          className="w-full py-4 bg-zinc-900 text-zinc-400 font-bold text-[10px] uppercase tracking-[0.3em] rounded-xl active:scale-[0.98] transition-all border border-white/5"
        >
          Agora não
        </button>
      </div>
