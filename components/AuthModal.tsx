        ...
        <div className="bg-[#0A0A0A]/80 backdrop-blur-2xl border border-[#FFD300]/10 rounded-2xl p-5 space-y-4">
          ...
          <div className="space-y-2.5">
            <button
              type="button"
              onClick={async () => {
                const res = await authService.signInWithSocial('apple');
                if (!res.ok) setErrors({ social: res.erro ?? 'Erro ao conectar.' });
              }}
              className="w-full min-h-[2.75rem] flex items-center justify-center gap-2.5 bg-white text-black font-bold text-[0.6875rem] uppercase tracking-[0.15em] rounded-xl active:scale-95 transition-all"
            >
            ...
            <button
              type="button"
              onClick={async () => {
                const res = await authService.signInWithSocial('google');
                if (!res.ok) setErrors({ social: res.erro ?? 'Erro ao conectar.' });
              }}
              className="w-full min-h-[2.75rem] flex items-center justify-center gap-2.5 bg-zinc-800 border border-white/10 text-white font-bold text-[0.6875rem] uppercase tracking-[0.15em] rounded-xl active:scale-95 transition-all"
            >
          ...
          <div className="flex items-center gap-3">
            ...
            <p className="text-white/30 text-[0.5625rem] font-bold uppercase tracking-widest">ou crie com e-mail</p>
            <div className="flex-1 h-px bg-white/10" />
          </div>
