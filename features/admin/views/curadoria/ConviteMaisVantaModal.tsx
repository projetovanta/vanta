...
export const ConviteMaisVantaModal: React.FC<{
  ...
  const [tier, setTier] = useState<TierMaisVanta>((tiers[0]?.id as TierMaisVanta) ?? ('CONVIDADO' as TierMaisVanta));
        ...
        <div className="px-6 pt-4" style={{ paddingBottom: 'max(2rem, env(safe-area-inset-bottom, 2rem))' }}>
          <div className="flex items-center gap-3 mb-6">
            ...
            <div className="min-w-0">
              ...
              <p className="text-zinc-400 text-[10px] mt-0.5 truncate">{membro.nome}</p>
            </div>
            <button onClick={onClose} className="ml-auto p-1.5 text-zinc-400 active:text-white shrink-0">
              <X size={14} />
            </button>
          </div>
          ...
          <div className="flex items-center gap-3 mb-5 p-3 bg-zinc-900/60 border border-white/5 rounded-2xl">
            ...
            <div className="min-w-0">
              ...
              <p className="text-zinc-400 text-[10px] truncate">{membro.email}</p>
              {membro.instagram && (
                <p className="text-zinc-400 text-[10px] truncate">@{membro.instagram.replace('@', '')}</p>
              )}
            </div>
          </div>
          ...
          {/* Seleção de tier */}
          <div className="mb-5">
            <p className="text-[8px] text-zinc-400 font-black uppercase tracking-widest mb-2">Tier</p>
