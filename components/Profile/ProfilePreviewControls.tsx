  ...
  return (
    <div className="flex justify-center gap-2 mb-4">
      <button
        ...
        className={`px-4 py-2 rounded-full text-xs font-bold transition-colors ${profilePreviewStatus === 'PUBLIC' ? 'bg-[#FFD300] text-black' : 'bg-zinc-800 text-zinc-400 hover:bg-zinc-700'}`}
      >
        Visualizar como Público
      </button>
      <button
        onClick={() => setProfilePreviewStatus('FRIENDS')}
        className={`px-4 py-2 rounded-full text-xs font-bold transition-colors ${profilePreviewStatus === 'FRIENDS' ? 'bg-[#FFD300] text-black' : 'bg-zinc-800 text-zinc-400 hover:bg-zinc-700'}`}
      >
