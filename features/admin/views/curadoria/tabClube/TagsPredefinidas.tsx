  ...
  return (
    <div className="px-1 space-y-1">
      <button
        ...
        className="flex items-center gap-1 text-zinc-500 text-[9px] font-bold uppercase tracking-wider"
      >
        <Tag size={9} /> Tags internas
        {expanded ? <ChevronUp size={9} /> : <ChevronDown size={9} />}
        {selected.length > 0 && (
          <span className="ml-1 bg-[#FFD300] text-black px-1.5 rounded-full text-[7px]">{selected.length}</span>
        )}
      </button>
      ...
      {expanded && (
        <div className="bg-zinc-900/80 border border-white/5 rounded-xl p-2 space-y-2 max-h-48 overflow-y-auto no-scrollbar">
          {Object.entries(TAGS_POR_CATEGORIA).map(([catId, cat]) => (
            <div key={catId}>
              <p className="text-zinc-500 text-[8px] font-black uppercase tracking-widest mb-1">{cat.label}</p>
                ...
                {cat.tags.map(tag => {
                  ...
                  return (
                    <button
                      ...
                      className={`px-2 py-0.5 rounded-full text-[9px] font-bold border transition-all active:scale-90 ${
                        isSelected
